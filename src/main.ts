...
function showNotice(message: string, _renderNow = true) {
  notice = message;
  noticeArea.textContent = message;
  noticeArea.dataset.visible = 'true';
  const currentMessage = message;
  window.setTimeout(() => {
    if (notice !== currentMessage) return;
    notice = '';
    noticeArea.textContent = '';
    delete noticeArea.dataset.visible;
  }, 4000);
}
...
function messageArea() { return `${error ? `<div class="error" role="alert">${esc(error)}</div>` : ''}`; }
function render(routeChanged = false) {
  ...
  updateRouteMetadata();
  app.innerHTML = `${demoBanner()}${nav(view)}<main id="main" tabindex="-1">${messageArea()}${content}</main><footer><span>Made for phrases from your life · <span class="spark" aria-hidden="true">✦</span> original generated illustration · ${BUILD_ID}</span><span class="footer-links"><a href="/privacy/">Privacy</a><a href="/terms/">Terms</a><span>Built by Param Factory</span></span></footer>`;
  bind();
  if (routeChanged) {
...
async function grade(remembered: boolean) {
  ...
  await savePhrase({ ...phrase, reviewStage: stage, nextReview: nextReviewAt(stage, reviewedAt), lastReviewed: reviewedAt.toISOString(), updatedAt: reviewedAt.toISOString() });
