...
function render() {
  ...
  document.title = demoMode ? 'Demo — Personal Vocab Loop' : 'Personal Vocab Loop — practice personal phrases';
  app.innerHTML = `${demoBanner()}${nav(view)}<main id="main" tabindex="-1">${noticeArea()}${content}</main><footer><span>Made for your own words · <span class="spark" aria-hidden="true">✦</span> illustration generated for this product</span><span><a href="/privacy/">Privacy</a><a href="/terms/">Terms</a></span></footer>`;
  bind();
}
...
function bind() {
  document.querySelector<HTMLButtonElement>('#reset-demo')?.addEventListener('click', async () => { await resetDemo(); filter = ''; location.hash = '#library'; showNotice('Sample phrases reset.'); await refresh(); });
